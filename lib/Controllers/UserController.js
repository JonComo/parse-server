...
var UserController = exports.UserController = function (_AdaptableController) {
  ...
  _createClass(UserController, [{
    ...
    value: function setPasswordResetToken(email) {
      var priv = this.config.database.find('Priv', { email: email });
      _logger.logger.info('found user with email ' + email + ' priv: ' + JSON.stringify(priv), null);
      if (priv && priv.length > 0) {
        return this.config.database.update('_User', { objectId: priv[0]['userId'] }, { _perishable_token: (0, _cryptoUtils.randomString)(25) }, {}, true);
      } else {
